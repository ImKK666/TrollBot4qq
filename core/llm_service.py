...
from bot_config import (FAST_LLM_MODEL_NAME, LLM_MODEL_NAME, OPENAI_API_KEY,
                        OPENAI_API_URL)
from services.memory_service import (add_conversation_snapshot,
            ...
            "properties": {
                "topic_analysis": {
                    ...
                    "properties": {
                        "theme": {
                            "type": "string",
                            "description": "对话主题的一句话总结。",
                        },
                        "summary": {
                            "type": "string",
                            "description": "对话内容的详细概要。",
                        },
                ...
                "social_dynamics_analysis": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "source_user_id": {
                                "type": "integer",
                                "description": "发起互动用户的QQ号。",
                            },
                            "target_user_id": {
                                "type": "integer",
                                "description": "被互动用户的QQ号。",
                            },
                            "attitude_description": {
                                "type": "string",
                                "description": "对source用户向target用户展现的行为和隐含态度的文字描述。",
                            },
                        },
                        "required": [
                            "source_user_id",
...
async def generate_fast_track_reply(
    target_message: Dict, context_messages: List[Dict]
) -> Union[str, None]:
    """
    快速通道回复生成逻辑
    1. 使用快速LLM评估消息是否值得回复。
    ...
    try:
        ...
        try:
            ...
            print(
                f"【快速通道】: 趣味性评估结果: {is_interesting}, 搜索词: '{search_query}'"
            )
        except json.JSONDecodeError:
            print(
                f"【快速通道】【错误】: 解析趣味性评估JSON失败。响应: {intent_response_raw}"
            )
    ...
    if search_query:  # 仅当有搜索词时才执行搜索
        try:
            memories_found = await search_relevant_memories(
                search_query, user_id=target_user_id
            )
            if memories_found:
    ...
    # 3. 准备最终回复的上下文
    formatted_context = "\n".join(
        [_format_message_for_prompt(msg) for msg in context_messages]
    )
    formatted_memories = "无"
    if relevant_memories:
        memory_lines = []
        for mem in relevant_memories:
            metadata = mem.get("metadata", {})
            timestamp = metadata.get("timestamp", "未知时间")
            summary = metadata.get("summary") or metadata.get("troll_potential")
            memory_lines.append(
                f"- [Doc {mem.get('id')}] 时间: {timestamp} 摘要: {summary or mem.get('original_text')}"
            )
        formatted_memories = "\n".join(memory_lines)
    ...
    try:
        ...
        if not final_reply_raw:
            if finish_reason == "content_filter":
                print(
                    "【快速通道】【警告】: LLM生成了空回复，原因是内容可能被服务商的安全策略过滤。"
                )
            elif finish_reason == "length":
                print(
                    "【快速通道】【警告】: LLM生成了空回复，原因是输出的内容达到了max_tokens的限制。"
                )
            else:
                print(
                    f"【快速通道】【警告】: LLM生成了空回复，但未提供明确原因 (finish_reason: {finish_reason})。"
                )
...
async def generate_proactive_attack_message(memory: Dict) -> Union[str, None]:
    ...
    try:
        ...
        if not attack_message:
            if finish_reason == "content_filter":
                print(
                    "【主动攻击】【警告】: LLM生成了空回复，原因是内容可能被服务商的安全策略过滤。"
                )
            else:
                print(
                    f"【主动攻击】【警告】: LLM生成了空回复 (finish_reason: {finish_reason})。"
                )
            return None
...
async def digest_messages_in_background(messages: List[Dict[str, Any]]):
    ...
    for msg in messages:
        ...
        conversation_history.append(
            {"role": "user", "content": [user_intro] + llm_message_content}
        )
    ...
    try:
        ...
        try:
            ...
            if topic_analysis:
                ...
                await add_conversation_snapshot(
                    group_id=group_id,
                    start_time=start_time,
                    end_time=end_time,
                    theme=topic_analysis.get("theme", ""),
            ...
            print(
                f"【错误】: LLM返回的不是有效的JSON格式。原始返回: {response_content}"
            )
