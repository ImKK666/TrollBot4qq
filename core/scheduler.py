...
from bot_config import (CRITICAL_TARGET_LIST,
                        PROACTIVE_ATTACK_ACTIVE_TIME_RANGE,
                        PROACTIVE_ATTACK_INACTIVITY_HOURS,
                        PROACTIVE_ATTACK_INTERVAL_SECONDS, TARGET_GROUP_IDS)
...
async def proactive_attack_task():
    ...
    while True:
        ...
        if not (start_hour <= now.hour < end_hour):
            print(
                f"【定时任务】: 当前时间 ({now.hour}点) 不在活跃时间段内，跳过本次检查。"
            )
            continue
        ...
        for user_id in CRITICAL_TARGET_LIST:
            ...
            if inactivity_seconds > PROACTIVE_ATTACK_INACTIVITY_HOURS * 3600:
                print(
                    f"【定时任务】: 发现目标 {user_id} 已沉默超过 {PROACTIVE_ATTACK_INACTIVITY_HOURS} 小时。"
                )
                ...
                if not memories:
                    print(
                        f"【定时任务】: 未找到用户 {user_id} 的任何黑历史，无法攻击。"
                    )
                    continue
                ...
                if attack_message:
                    ...
                    print(
                        f"【定时任务】: 决定在群 {target_group_id} 对用户 {user_id} 发动攻击！"
                    )
