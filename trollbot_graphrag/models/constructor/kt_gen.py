...
class KTBuilder:
    ...
    def save_chunks_to_file(self):
        ...
        if os.path.exists(chunk_file):
            try:
                ...
                logger.warning(
                    f"Failed to parse existing chunks from {chunk_file}: {type(e).__name__}: {e}"
                )
    ...
    def _process_attributes(
        self, extracted_attr: dict, chunk_id: int, entity_types: dict = None
    ) -> tuple[list, list]:
    ...
    def _process_triples(
        self, extracted_triples: list, chunk_id: int, entity_types: dict = None
    ) -> tuple[list, list]:
    ...
    def _connect_keywords_to_communities(self):
        ...
        kw_nodes = [
            n for n, d in self.graph.nodes(data=True) if d["label"] == "keyword"
        ]
        with self.lock:
