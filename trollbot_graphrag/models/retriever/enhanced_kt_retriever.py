...
from .faiss_filter import DualFAISSRetriever
    ...
    def _load_node_embedding_cache(self):
        ...
        if os.path.exists(cache_path_npz):
            try:
                ...
                logger.info(
                    f"Loading node embedding cache from {cache_path_npz} (file size: {file_size} bytes)"
                )
        ...
        if os.path.exists(cache_path):
            try:
                ...
                logger.info(
                    f"Loaded node embedding cache with {len(self.node_embedding_cache)} entries from {cache_path} (file size: {file_size} bytes)"
                )
                return True
