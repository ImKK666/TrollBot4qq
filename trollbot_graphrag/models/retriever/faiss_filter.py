...
class DualFAISSRetriever:
    def __init__(
        ...
        if device is not None:
            if device == "cuda" and not torch.cuda.is_available():
                logger.warning(
                    "Warning: CUDA requested but not available in DualFAISSRetriever, falling back to CPU"
                )
    ...
    def _calculate_node_scores(self, query_embed, nodes: List[str]) -> Dict[str, float]:
        ...
        for node in nodes:
            if "embedding" in self.graph.nodes[node]:
                ...
                scores[node] = F.cosine_similarity(
                    query_tensor, self.node_embedding_cache[node], dim=0
                ).item()
            else:
        ...
        if nodes_with_embedding:
            ...
            for node in nodes_with_embedding:
                node_embed = torch.FloatTensor(self.graph.nodes[node]["embedding"]).to(
                    self.device
                )
            ...
            if embeddings:
                ...
                similarities = F.cosine_similarity(
                    query_tensor.unsqueeze(0), embeddings_tensor, dim=1
                )
        ...
        if nodes_to_encode:
            ...
            if texts:
                ...
                similarities = F.cosine_similarity(
                    query_tensor.unsqueeze(0), node_embeddings, dim=1
                )
        ...
        if nodes_to_encode:
            ...
            if texts:
                try:
                    ...
                    similarities = F.cosine_similarity(
                        query_tensor.unsqueeze(0), embeddings, dim=1
                    )
    ...
    def save_embedding_cache(self):
        ...
        try:
            ...
            logger.info(
                f"Saved embedding cache with {len(numpy_cache)} entries to {cache_path} (size: {file_size} bytes)"
            )
            return True
    ...
    def load_embedding_cache(self):
        ...
        if os.path.exists(cache_path):
            try:
                ...
                logger.info(
                    f"Loaded embedding cache with {len(self.node_embedding_cache)} entries from {cache_path} (file size: {file_size} bytes)"
                )
                return True
        ...
        if all_exist:
            try:
                ...
                if os.path.exists(dim_transform_path):
                    try:
                        cached_dim_info = torch.load(
                            dim_transform_path, map_location="cpu", weights_only=False
                        )
                        cached_model_dim = cached_dim_info.get("model_dim")
                        if cached_model_dim != self.model_dim:
                            logger.info(
                                f"Model dimension changed: cached {cached_model_dim}, current {self.model_dim}"
                            )
                            dim_consistent = False
                    except Exception as e:
                        logger.warning(
                            f"Error checking dimension transform consistency: {e}"
                        )
                        dim_consistent = False
    ...
    def _load_dim_transform(self):
        ...
        try:
            ...
            if cached_model_dim != self.model_dim:
                logger.warning(
                    f"Model dimension mismatch: cached {cached_model_dim}, current {self.model_dim}"
                )
                return False
            ...
            if has_transform and "state_dict" in save_data:
                ...
                if self.dim_transform is None:
                    logger.info(
                        "No dimension transform needed (cached and current both 384-dim)"
                    )
                    return True
                else:
    ...
    def _build_node_index(self):
        ...
        torch.save(
            self.node_embeddings, f"{self.cache_dir}/{self.dataset}/node_embeddings.pt"
        )
    ...
    def _build_community_index(self):
        ...
        communities = {
            n for n, d in self.graph.nodes(data=True) if d.get("label") == "community"
        }
            ...
            try:
                ...
                try:
                    self.node_embeddings = torch.load(
                        node_embed_path, weights_only=False
                    )
                except TypeError:
            ...
            try:
                ...
                try:
                    self.relation_embeddings = torch.load(
                        relation_embed_path, weights_only=False
                    )
                except TypeError:
    ...
    def _verify_data_consistency(self):
        ...
        if missing_in_embeddings:
            logger.warning(
                f"Warning: {len(missing_in_embeddings)} nodes in graph but not in embeddings: {list(missing_in_embeddings)[:5]}..."
            )

        if extra_in_embeddings:
            logger.warning(
                f"Warning: {len(extra_in_embeddings)} nodes in embeddings but not in graph: {list(extra_in_embeddings)[:5]}..."
            )
        ...
        if not missing_in_embeddings and not extra_in_embeddings:
            ...
            logger.info(
                f"✗ Data inconsistency detected: {len(missing_in_embeddings)} missing, {len(extra_in_embeddings)} extra"
            )
    ...
    def _format_community_content(
        self, base_text: str, entities: list[str], keywords: list[str]
    ) -> str:
    ...
    def _nodes_to_text(self, nodes: List[str]) -> str:
        ...
        for node in nodes:
            ...
            if node_type == 2:  # Entity
                ...
                community_text = self._format_community_content(
                    base_text, entities, keywords
                )
