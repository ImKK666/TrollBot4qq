...
class ConfigManager:
    ...
    def _parse_config(self) -> None:
        ...
        self.datasets = {
            name: DatasetConfig(**config) for name, config in datasets_data.items()
        }
    ...
    def _validate_config(self) -> None:
        ...
        for dataset_name, dataset_config in self.datasets.items():
            if not os.path.exists(dataset_config.corpus_path):
                logger.warning(
                    f"Corpus path not found for {dataset_name}: {dataset_config.corpus_path}"
                )
            if not os.path.exists(dataset_config.schema_path):
                logger.warning(
                    f"Schema path not found for {dataset_name}: {dataset_config.schema_path}"
                )

        valid_modes = ["agent", "noagent"]
        if self.triggers.mode not in valid_modes:
            raise ValueError(
                f"Invalid mode: {self.triggers.mode}. Must be one of {valid_modes}"
            )
    ...
    def get_prompt_formatted(self, category: str, prompt_type: str, **kwargs) -> str:
        ...
        try:
            ...
            raise ValueError(
                f"Missing variable {e} for prompt {category}.{prompt_type}"
            )
